# Wire an agent (MCP)

Connect any MCP-capable agent to one governed endpoint (see `mcp.json`); also shown as an HTTP tool-calling loop.

```bash
python recipes/08-mcp-agent/main.py
```

Reads `BHARATROUTER_API_KEY` from the environment and points at `https://api.bharatrouter.com/v1`.
Set `BHARATROUTER_MODEL` to any id from [the catalog](https://bharatrouter.com/models), or let the
recipe pick one for you.

**Web version:** https://bharatrouter.com/cookbook/mcp-agent
